    ...
    <>
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <h1 className="text-lg font-semibold md:text-lg">Project: {project?.title}</h1>
          <div className="flex gap-2">
            <p className="text-xs"><b>Method:</b> {METHOD_MAPPING[project.projection_method]}</p>
            <p className="text-xs"><b>Industry:</b> {project.industry}</p>
            <p className="text-xs"><b>Business Focus:</b> {project.businessFocus}</p>
          </div>
        </div>

        {showEmptyState ? (<form className='flex gap-2' action={handleSubmitFile}>
          <Input className="max-w-80" type="file" name="pdfFile" accept=".pdf" required />
          ...
          <div className="flex gap-2">
            ...
            {(generating || loading) && (
              ...
              <div className="flex gap-1 items-center mr-2 animate-pulse">
                <Image className="" width={32} height={32} src="/images/logo.jpeg" alt='Intelligis logo' />
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                      ...
                      />
                      <Button type="submit" size="sm" disabled={generating}>{generating ? 'Generating...' : 'Send'}</Button>
                    </form>
                  </Form>
                </div>
              </PopoverContent>
            ...
            <Popover>
              ...
              <PopoverContent className="min-w-80">
                <div className="flex flex-col gap-2">
                  <form className='flex flex-col gap-2' action={handleSubmitFile}>
                    <Input type="file" name="pdfFile" accept=".pdf" required />
                    <Button type="submit" size="sm" disabled={generating}>{generating ? 'Generating...' : 'Generate'}</Button>
                  </form>
                </div>
              </PopoverContent>
            </Popover>

            <Button size="sm" variant="outline" className='ml-auto gap-1.5' onClick={() => setHistoryOpen(!historyOpen)}>
      ...
      <div>
        <main className='grid flex-1 gap-4 overflow-hidden md:grid-cols-1 lg:grid-cols-5'>
          {/* Project view */}
          {historyOpen && <History project={project} setCurrent={setCurrentProjection} />}
          <div
            className={cn(
          ...
          >
            <div className='relative'>
              <ScrollArea className='h-[calc(100vh-210px)]'>
                <div
                  className={cn(
                    'grid grid-cols-1 md:grid-cols-4 gap-8 mb-8',
                    { ['md:grid-cols-3']: adjustmentsOpen || historyOpen },
                  )}
                >
                  {totalsToDisplay?.slice(0, 4)?.map((item, idx) => (
                    <StatCard
                      key={idx}
                      label={item.title}
